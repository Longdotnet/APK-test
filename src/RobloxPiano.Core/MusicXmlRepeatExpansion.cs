...
/// Supported semantics are deterministic two-pass forward/backward repeats with optional first/second endings.
/// Nested repeats, repeat counts other than two, malformed ending ranges and ending numbers other than 1/2 fail closed.
/// </summary>
public static class MusicXmlRepeatExpansion
{
    ...
    private static IReadOnlyList<XElement> ExpandPart(IReadOnlyList<XElement> measures, string? partId)
    {
        ...
        var pass = 1;
        var guard = 0;
        var maxSteps = Math.Max(32, measures.Count * 6);

        while (index < measures.Count)
        {
            if (++guard > maxSteps)
            {
                throw Error(partId, index, "Repeat expansion exceeded the deterministic safety bound; nested or cyclic repeat structure is not supported.");
            }

            var current = metadata[index];
            if (current.ForwardRepeat)
            {
                if (repeatActive && pass == 1 && index != repeatStart)
                {
