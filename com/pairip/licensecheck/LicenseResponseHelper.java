...
public class LicenseResponseHelper {
    ...
    private static PublicKey getPublicKey() throws LicenseCheckException {
        try {
            return KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(Base64.decode(LicenseClient.getLicensePubKey(), 0)));
        } catch (IllegalArgumentException e) {
            throw new LicenseCheckException("Could not decode public key", e);
        } catch (NoSuchAlgorithmException e2) {
            throw new LicenseCheckException(String.format("%s algorithm not found on device", "RSA"), e2);
