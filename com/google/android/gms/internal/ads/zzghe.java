package com.google.android.gms.internal.ads;

/* JADX INFO: loaded from: classes.dex */
public final class zzghe extends zzgfz {
    private final zzghm zza;
    private final zzgxf zzb;
    private final zzgxe zzc;
    private final Integer zzd;

    public /* synthetic */ zzghe(zzghm zzghmVar, zzgxf zzgxfVar, zzgxe zzgxeVar, Integer num, zzghd zzghdVar) {
        this.zza = zzghmVar;
        this.zzb = zzgxfVar;
        this.zzc = zzgxeVar;
        this.zzd = num;
    }

    public static zzghc zzc() {
        return new zzghc(null);
    }

    @Override // com.google.android.gms.internal.ads.zzgfz, com.google.android.gms.internal.ads.zzgez
    public final /* synthetic */ zzgfm zza() {
        return this.zza;
    }

    @Override // com.google.android.gms.internal.ads.zzgfz
    public final zzgxe zzb() {
        return this.zzc;
    }

    public final zzghm zzd() {
        return this.zza;
    }

    public final zzgxf zze() {
        return this.zzb;
    }

    public final Integer zzf() {
        return this.zzd;
    }
}
