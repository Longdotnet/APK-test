package com.google.android.gms.internal.ads;

/* JADX INFO: loaded from: classes.dex */
public final class zzggu extends zzgfz {
    private final zzghb zza;
    private final zzgxf zzb;
    private final zzgxe zzc;
    private final Integer zzd;

    public /* synthetic */ zzggu(zzghb zzghbVar, zzgxf zzgxfVar, zzgxe zzgxeVar, Integer num, zzggt zzggtVar) {
        this.zza = zzghbVar;
        this.zzb = zzgxfVar;
        this.zzc = zzgxeVar;
        this.zzd = num;
    }

    public static zzggs zzc() {
        return new zzggs(null);
    }

    @Override // com.google.android.gms.internal.ads.zzgfz, com.google.android.gms.internal.ads.zzgez
    public final /* synthetic */ zzgfm zza() {
        return this.zza;
    }

    @Override // com.google.android.gms.internal.ads.zzgfz
    public final zzgxe zzb() {
        return this.zzc;
    }

    public final zzghb zzd() {
        return this.zza;
    }

    public final zzgxf zze() {
        return this.zzb;
    }

    public final Integer zzf() {
        return this.zzd;
    }
}
