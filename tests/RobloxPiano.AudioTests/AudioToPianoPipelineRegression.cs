...
internal static class AudioToPianoPipelineRegression
{
    public static void RealModelSineProducesCanonicalPerformance()
    {
        ...
        True(result.Arrangement.Track.Events.Count > 0);
        True(result.Arrangement.Track.TimelineLength > TimeSpan.Zero);
        True(result.Arrangement.Diagnostics.ArrangedEvents > 0);
        True(result.Diagnostics.TotalElapsed > TimeSpan.Zero);
    }

    public static void WrongNormalizedRateFailsBeforeInference()
    {
        var modelPath = RequireModelPath();
    ...
    public static void PreCancelledPipelineStopsBeforeInference()
    {
        var modelPath = RequireModelPath();
        using var service = new AudioToPianoTranscriptionService(modelPath);
    ...
    private static float[] BuildReferenceTone(double seconds, double frequencyHz)
    {
